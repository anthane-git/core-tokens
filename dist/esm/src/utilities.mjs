...
function getKeyframeNames(motionTokenGroup) {
  return Object.keys(motionTokenGroup).map(token => token.startsWith('keyframes') ? `core-${token}` : null).filter(Boolean);
}
/**
...
function getCustomPropertyNames(tokens) {
  return Object.entries(tokens).map(([, tokenGroup]) => Object.keys(tokenGroup).map(token => createVar(token))).flat();
}
